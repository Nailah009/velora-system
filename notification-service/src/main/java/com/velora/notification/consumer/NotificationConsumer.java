...
public class NotificationConsumer {
    ...
    @KafkaListener(topics = "${app.kafka.topics.shipment-failed}", groupId = "notification-shipment-failed")
    public void onShipmentFailed(String message) throws Exception {
        printIncoming("NOTIFICATION CONSUMER - RECEIVE SHIPMENT FAILED", "notification.shipment.failed.queue", message);
        notificationService.saveNotification("SHIPMENT_FAILED", message);
    }

    @KafkaListener(topics = "${app.kafka.topics.payment-refunded}", groupId = "notification-payment-refunded")
    public void onPaymentRefunded(String message) throws Exception {
        printIncoming("NOTIFICATION CONSUMER - RECEIVE PAYMENT REFUNDED", "notification.payment.refunded.queue", message);
        notificationService.saveNotification("PAYMENT_REFUNDED", message);
    }
