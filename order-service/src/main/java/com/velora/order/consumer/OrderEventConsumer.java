...
public class OrderEventConsumer {
    ...
    @KafkaListener(topics = "${app.kafka.topics.shipment-failed}", groupId = "order-service-shipment-failed")
    public void onShipmentFailed(String message) throws Exception {
        JsonNode event = objectMapper.readTree(message);
        Long orderId = event.get("orderId").asLong();
        orderService.markShipmentFailed(orderId);
        printStatusUpdate("ORDER SERVICE - RECEIVE SHIPMENT FAILED", orderId, "CANCELLED", event);
    }
